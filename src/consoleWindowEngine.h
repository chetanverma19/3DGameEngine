...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&consoleWindowEngine::GameThread, this);
	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				// Handle Frame Update
				if (!OnWindowUpdate(fElapsedTime))
					m_bAtomActive = false;
...
public:
	...
	virtual bool OnWindowUpdate(float fElapsedTime) = 0;
...
// Just check if wave format is compatible with olcCGE
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&consoleWindowEngine::AudioThread, this);
	...
	{
		((consoleWindowEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> consoleWindowEngine::m_bAtomActive(false);
std::condition_variable consoleWindowEngine::m_cvGameFinished;
std::mutex consoleWindowEngine::m_muxGame;
